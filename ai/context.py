...
_PAYMENT_GUARD_TTL_SECONDS: Final = 30 * 24 * 60 * 60


def _payment_guard_key(payment_id: str) -> str:
    """Ключ-страж против двойной активации одного платежа ЮKassa
    (клиент может нажать «Проверить оплату» несколько раз)."""
    return f"{HISTORY_KEY_PREFIX}payment:{payment_id}"
...
class HistoryStore:
    ...
    async def try_mark_payment(self, payment_id: str) -> bool:
        """SETNX-страж против двойной активации платежа ЮKassa. Возвращает
        True при первом вызове для данного payment_id (можно активировать),
        False если платёж уже был обработан."""
        ok = await self._r.set(
            _payment_guard_key(payment_id), "1", nx=True, ex=_PAYMENT_GUARD_TTL_SECONDS
        )
        return bool(ok)
