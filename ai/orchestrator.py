...
Single point of contact with the upstream chat-completions API. Supports
two providers under one ``chat()`` signature:

- ``yc``         — Yandex AI Studio Foundation Models
                   (``https://llm.api.cloud.yandex.net/v1``), auth via
                   ``Api-Key`` + ``x-folder-id`` header. Primary tier.
                   Models are referenced via the full Yandex URI
                   ``gpt://<folder>/<model>/latest``.
- ``openrouter`` — OpenRouter (``https://openrouter.ai/api/v1``),
                   auth via ``Bearer``. Tier-2 emergency fallback.
...
Provider = Literal["yc", "openrouter"]

_YC_BASE_URL: Final = "https://llm.api.cloud.yandex.net/v1"
...
def _get_client(provider: Provider) -> httpx.AsyncClient:
    ...
    if provider == "yc":
        client = httpx.AsyncClient(
            base_url=_YC_BASE_URL,
            headers={
                "Authorization": f"Api-Key {settings.yc_ai_api_key.get_secret_value()}",
                "x-folder-id": settings.yc_ai_folder_id,
