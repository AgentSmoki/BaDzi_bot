...
from bot.services.yookassa_api import yookassa_api_live
...
def _free_questions_used_msg(limit: int, *, live: bool) -> str:
    ...
    вопрос», сейчас лимит ≥ 3. ``live`` — подключена ли реальная оплата
    (ЮKassa): при live нет варианта «продолжить бесплатно»."""
    head = f"Вы использовали все {limit} бесплатных вопросов. "
    if live:
        return head + "Чтобы продолжить диалог с Мастером Шифу, выберите тариф ниже."
    return head + (
        "Чтобы продолжить диалог с Мастером Шифу, выберите тариф ниже — "
        "или продолжите бесплатно (пока подключаем оплату)."
    )
...
async def handle_ask_pressed(
    ...
    if chart is None:
        await callback.message.answer(
            "Сначала постройте карту — Мастер Шифу отвечает только на её основе.",
    ...
    *,
    ...
    if chart is None or message.bot is None:
        await message.answer(
            "Не нашёл карту — вернитесь в меню и постройте её заново.",
            reply_markup=_no_chart_kb(),
        )
        await state.set_state(None)
        return
...
async def handle_partner_skip(
    ...
    if chart is None or not original_question:
        await callback.message.answer(
            "Не нашёл исходный вопрос — задайте его заново через меню.",
            reply_markup=_no_chart_kb(),
        )
        await state.set_state(None)
        await callback.answer()
...
async def handle_suggested_followup(
    ...
    if not suggested:
        await callback.message.answer(
            "Не нашёл предложенный вопрос — задайте его текстом, и я отвечу.",
    ...
    if chart is None or callback.message.bot is None:
        await callback.message.answer(
            "Не нашёл активную карту — откройте её через меню.",
            reply_markup=_no_chart_kb(),
        )
        await state.set_state(None)
        return
...
async def handle_pay_buy(callback: CallbackQuery) -> None:
    ...
    if spec is None or callback.message.bot is None:
        await callback.answer("Оплата временно недоступна.", show_alert=True)
        return
    price, _days, label = spec

    # Приоритет — ЮKassa API (даёт СБП). Нативный инвойс остаётся фолбэком.
    if yookassa_api_live(settings):
        await start_yookassa_payment(
            callback.message.bot,
            callback.message.chat.id,
            amount_rub=price,
            title=f"Безлимит вопросов — {label}",
            description=f"Безлимитные вопросы Мастеру Шифу, тариф «{label}».",
            metadata={"kind": "q", "plan": plan},
        )
        await callback.answer()
        return

    token = settings.telegram_payment_provider_token
    if not payments_live(settings) or token is None:
        await callback.answer("Оплата временно недоступна.", show_alert=True)
        return
    await send_invoice(
        callback.message.bot,
...
async def handle_voice_question(
    ...
    if chart is None or message.bot is None:
        await message.answer(
            "Не нашёл карту — постройте её через меню и повторите вопрос.",
            reply_markup=_no_chart_kb(),
        )
        await state.set_state(None)
        return
    ...
    if remaining <= 0:
        ...
        await message.answer(
            _free_questions_used_msg(settings.free_questions_limit, live=payments_active(settings)),
            reply_markup=pricing_kb(payments_active=payments_active(settings)),
        )
        logger.info(
...
async def _continue_consultation_with_skill(
    ...
    *,
    ...
    try:
        ...
        await message.answer(
            "Мастер Шифу не смог ответить — попробуйте задать вопрос ещё раз.",
            reply_markup=_after_answer_kb(),
        )
        return
    finally:
