...
from bot.services.yookassa_api import get_payment
...
async def _activate_question_plan(
    ...
    await message.answer(
        ...
        f"Безлимитные вопросы Мастеру Шифу до {expires_at.strftime('%d.%m.%Y')}. "
...
# ── pay:check:* — подтверждение платежа ЮKassa REST API (Вариант B / СБП) ──


@payments_router.callback_query(F.data.startswith("pay:check:"))
async def handle_pay_check(
    callback: CallbackQuery,
    session: AsyncSession,
    user: User,
    history_store: HistoryStore,
) -> None:
    """Клиент жмёт «Проверить оплату» после формы ЮKassa. Спрашиваем
    статус платежа; при ``succeeded`` активируем подписку из metadata
    (один раз — redis-страж от двойного нажатия)."""
    ...
    try:
        pay = await get_payment(settings, payment_id)
    except httpx.HTTPError as exc:
        logger.warning("payment.check_failed", payment_id=payment_id, error=str(exc))
        await callback.answer(
            "Не получилось проверить оплату. Попробуй ещё раз через минуту.", show_alert=True
        )
        return

    if pay["status"] != "succeeded":
        await callback.answer(
            "Оплата пока не подтверждена. Если ты только что оплатил — "
            "подожди минуту и нажми «Проверить» снова.",
            show_alert=True,
        )
        return

    if not await history_store.try_mark_payment(payment_id):
    ...
    kind = meta.get("kind", "")
    if kind == "q":
        parts = ["q", meta.get("plan", "")]
        await _activate_question_plan(callback.message, session, user, parts, payment_id)
    elif kind in ("fm", "fd"):
        parts = [kind, meta.get("arg", ""), meta.get("school", ""), meta.get("chart_id", "")]
        await _activate_forecast(callback.message, session, user, kind, parts, payment_id)
    else:
        logger.warning("payment.check_unknown_meta", payment_id=payment_id, meta=meta)
    await callback.answer()
