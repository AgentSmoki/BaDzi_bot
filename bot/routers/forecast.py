...
async def _proceed_daily(
    ...
    if message.bot is None:
        return
    _daily_desc = (
        f"Дневной прогноз + активации на 30 дней, ежедневно в {hour_local:02d}:00 вашего времени."
    )
    if yookassa_api_live(settings):
        await start_yookassa_payment(
            message.bot,
