...
_PRICING_HEAD = (
    "<b>Тарифы Мастера Шифу</b>\n\n• Месяц — 290 ₽\n• 3 месяца — 990 ₽\n• Год — 2 490 ₽\n\n"
)
# Текст когда ЮKassa подключена (payments_live) vs пока на free-bypass.
PRICING_TEXT_LIVE = _PRICING_HEAD + "Безлимитные вопросы Мастеру Шифу на выбранный срок."
PRICING_STUB_TEXT = _PRICING_HEAD + "Оплата подключается — пока пользуйтесь бесплатным вопросом."
...
# Прозрачный webp-стикер Мастера Шифу (приветствие). Лежит в примонтированном
# ro-томе ./assets. Отправляется первым при первом контакте новичка.
_SHIFU_GREETING_STICKER = "assets/stickers/shifu_greeting.webp"

GREETING_NEW_USER = (
    "Здравствуй, {name}. Я — Мастер Шифу. Много лет я читаю судьбу по "
    "системе Ба Цзы — как читают карту местности.\n\n"
    "В час твоего рождения небо оставило отпечаток: четыре столпа, восемь "
    "знаков. По ним я вижу твои сильные стороны, ритмы удачи, добрые годы "
    "и узкие места на пути.\n\n"
    "<blockquote><i>Ба Цзы — это искусство оказаться в нужное время, в "
    "нужном месте и с нужными людьми.</i></blockquote>\n\n"
    "Расчёт карты и первое прочтение — мой подарок тебе, бесплатно. "
    "Помогу разглядеть рисунок, по которому идёт твоя жизнь.\n\n"
    "Чтобы начать, назови то, что помнишь о своём рождении: дату, время и город.\n\n"
    "Начнём?"
)
...
async def handle_start(
    ...
    await message.answer(
        "👋 Кнопки быстрого доступа закрепил снизу.",
        reply_markup=main_reply_kb(),
    )

    if not charts:
        logger.info("start.new_user", telegram_id=user.telegram_id, user_id=str(user.id))
        # Мастер Шифу «здоровается» стикером перед приветствием. Файл может
        # отсутствовать (свежий деплой) — тогда просто пропускаем стикер.
        with contextlib.suppress(TelegramBadRequest, FileNotFoundError):
            await message.answer_sticker(FSInputFile(_SHIFU_GREETING_STICKER))
...
async def handle_menu_pricing(callback: CallbackQuery) -> None:
    ...
    """
    from bot.services.payments import payments_active

    live = payments_active(get_settings())
    if isinstance(callback.message, Message):
        await callback.message.answer(
            PRICING_TEXT_LIVE if live else PRICING_STUB_TEXT,
            reply_markup=pricing_kb(payments_active=live),
        )
    await callback.answer()


# NB: реальные кнопки оплаты шлют ``pay:buy:*`` / ``pay:disabled:*`` и
# обрабатываются в consultation.py (handle_pay_buy / handle_payment_disabled).
# Старый catch-all ``pay:*`` здесь удалён — он перехватывал ``pay:buy:*``
# раньше consultation_router (start_router подключён первым) и глушил инвойс.
...
async def handle_chart_interpret(
    ...
    if chart is None:
        await callback.message.answer(
            "Не нашёл карту — постройте её через меню и повторите.",
    ...
    try:
        ...
        await callback.message.answer(
            "Мастер Шифу не смог собрать разбор. Попробуйте ещё раз через минуту.",
            reply_markup=chart_actions_kb(chart_id=chart.id),
        )
        return
    finally:
