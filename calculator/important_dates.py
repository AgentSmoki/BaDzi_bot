...
def format_important_date_message(
    ...
    star_lines = "\n".join(f"  • {_star_label(sid)}" for sid in important.active_stars)
    ...
    return (
        ...
        f"{star_lines}\n\n"
        f"{severity_note}\n\n"
        "В конце дня я предложу записать рефлексию — что прожилось "
        "на этой энергии. Можно отказаться, тогда я просто отмечу день "
        "в дневнике автоматически.\n\n"
