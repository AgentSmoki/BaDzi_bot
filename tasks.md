...
- [ ] 2.1.5 calculator/structure.py — Специальные структуры карты (格局)
  - **TODO (отложено из 2.1.3, обработать в 2.1.5 или отдельным модулем):**
    - 元辰 (Юань Чэнь) — динамическая формула: для Ян-года ветвь = (clash_branch + 1), для Инь-года = (clash_branch − 1). Требует определения чётности `year_stem` и расчёта противостоящей ветви.
    - 勾绞 (Гоу Цзяо) — динамическая формула ±3: для Ян-года Крюк = year_branch + 3, Петля = year_branch − 3; для Инь-года наоборот.
    - 空亡 (Кун Ван) — требует расчёта Сюнь (декада 60-цикла) от столпа дня. Таблица: 甲子旬→戌亥, 甲戌旬→申酉, 甲申旬→午未, 甲午旬→辰巳, 甲辰旬→寅卯, 甲寅旬→子丑.
    - Источник правил: [doc/research/symbolic_stars_v2_gemini.md](doc/research/symbolic_stars_v2_gemini.md), строки 39-41 в CSV.
