...
| -------------- | ------------------------------------- | ---------------------------------------------------- |
| AI провайдер   | OpenRouter API                        | Один ключ: Kimi + Claude + gpt-image-1               |
| AI основная    | Kimi K2 (moonshot)                    | Специализация на китайских текстах (Бацзы)           |
| AI fallback    | Claude 3.5 Sonnet                     | Резервная при недоступности Kimi                     |
| Визуал карты   | gpt-image-1 (OpenRouter)              | Высококачественная генерация в стиле Mingli          |
| Ассеты         | 24 PNG иероглифов (подготовить)       | Иероглифы как картинки, а не Unicode текст           |
...
| Тарифы         | Разовый 99р / Сессия 349р / Подписка 690р/мес | Unit-экономика: ~1-6 RUB/запрос через Kimi |
...
| Основная модель  | Kimi K2 (moonshot/moonshot-kimi-k2)    |
| Резервная модель | Claude 3.5 Sonnet (anthropic)          |
| Генерация карты  | gpt-image-1 (openai, через OpenRouter) |
...
|--------|----------|--------|
| 24 PNG ассета иероглифов | 10 стволов + 12 ветвей + 2 Инь/Ян в стиле Mingli | ⏳ Нужно подготовить |
| Swiss Ephemeris данные | JPL DE431 ephe files (se1-ephe.zip) | ⏳ Скачать |
| OpenRouter API key | Зарегистрироваться, пополнить баланс | ⏳ |
| Yandex Cloud ресурсы | VPS + PostgreSQL + Redis + Object Storage | ⏳ |
...
| Тариф     | Цена       | Включено                                   |
| --------- | ---------- | ------------------------------------------ |
| Free      | 0 руб.     | 3 вопроса/день, расчёт карты, визуализация |
| Месяц     | 490 руб.   | Безлимит, все темы, история консультаций   |
